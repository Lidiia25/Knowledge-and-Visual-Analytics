...
client = Socrata('health.data.ny.gov',
                 ...
                 username='lida.tronina@gmail.com',
                 password='Tronina1303$')
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.Div('The data for this visualization is from the New York State Department of Health. It contains discharge level details on patient characteristics, diagnoses, treatments, services, and charges for 2017 Hospital Inpatient discharges in New York State. This web app designed to display information directly from the DOH API. It shows top hospital and procedure costs in New York City and allows a user to choose region, types of admissions, diagnosis categories, etc. It  shows a high-cost trending procedure and providers by total cost and average cost per procedure. Table shows high cost claims(outliers) in each selection.'),
            html.Div([
        ...
        ], style={'width': '350px', 'flex': '1', 'padding': '15px'}),
        html.Div([
...
def handle(hospital_service_area,
    ...
    return {
        'data': [trace],
        'layout': go.Layout(
            title='Hospital total cost',
            xaxis={'title': 'Facility name'},
...
def handle(hospital_service_area,
    ...
    if len(results) == 0:
        ...
        trace = go.Bar(
            x=df['ccs_procedure_description'],
            y=df['sum_total_costs'],
        )
...
def handle(hospital_service_area,
    ...
    if len(results) == 0:
        ...
        return df_to_table(pd.DataFrame.from_records(results))
